...
# ------------------- CONFIG -------------------
MODEL_DIR = "/home/gadde/Thesis/experiments/led-base-16384/nospecialtokens/2025_07_30-11:10:30_lr5e-06_bs1_epochs10_run001/checkpoints" 
MODEL_PATH = os.path.join(MODEL_DIR, "best_model.pth")
TOKENIZER_PATH = "/home/gadde/Thesis/models/pretrained/led-base-16384"

INPUT_JSON = "/home/gadde/Thesis/data/infer/incomplete_srs_00214.json"
OUTPUT_DIR = "/home/gadde/Thesis/src/led-base-16384/inference/no_special_tokens/2025_07_30-11:10:30_lr5e-06_bs1_epochs10_run001/output"
...
def generate_output(model, tokenizer, inputs, global_attention_mask):
    ...
    decoded_raw = tokenizer.batch_decode(generated_ids, skip_special_tokens=True)[0]
...
def save_output_as_json(text_output, output_path):
    with open(output_path.replace(".json", ".txt"), "w") as f:
        f.write(text_output)
    logger.info(f"✓ Output saved as raw text at: {output_path.replace('.json', '.txt')}")
...
def main():
    logger.info("Starting inference (no special tokens)...")
    model, tokenizer = load_model_and_tokenizer()
    inputs, global_attention_mask = prepare_input(INPUT_JSON, tokenizer)
    output_clean, output_raw = generate_output(model, tokenizer, inputs, global_attention_mask)
    save_output_as_json(output_clean, OUTPUT_JSON)
